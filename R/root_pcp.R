...
#' constraint.
#' @inheritParams rrmc
#' @param lambda,mu (Optional) A pair of doubles regularizing the recovery of
#'   the `L` and `S` estimates. `lambda` controls the sparsity of the output `S`
#'   matrix; larger values penalize non-zero entries in `S` more stringently,
#'   driving the recovery of sparser `S` matrices. `mu` adjusts the model's
#'   sensitivity to noise; larger values will penalize errors between the
#'   predicted model and the observed data more severely. It is highly
...
root_pcp <- function(D, lambda = NULL, mu = NULL, LOD = -Inf, non_negative = TRUE, max_iter = 10000, verbose = FALSE) {
  ...
  for (i in 1:max_iter) {
    ...
    L3_old <- L3
    temp <- L2 + S2 - Y3 / rho
    temp_D <- D * mask_above_LOD + temp * (mask_below_LOD & (temp >= 0) & (temp <= LOD)) + ifelse(mask_below_LOD & (temp > LOD), LOD, 0)
