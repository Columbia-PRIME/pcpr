#' @export
pcp_rank_r_nm_adaptive_gamma_no_t = function(D, gamma, r, verbose = FALSE) {
  #% solve the problem
  #%
  ...
  while (!done) {
    ...
    if (iter >= max_iter || delta < step_tol) done = TRUE
    if (verbose) {
      if (iter %% 100 == 0) print(paste0('Iter: ', iter, ', Obj: ', round(obj,4), ', Step: ', round(delta,4)))
      if (iter >= max_iter) print('Did not converge.')
      if (delta < step_tol & iter < max_iter) print(paste("Converged in:", iter, "iterations."))
    }
