...
* As we saw in the heatmap, the scale for each chemical's measured concentration differs widely (e.g., NH4 ranges from 0 to
  9 µg/m$^3$, while Ti ranges from 0 to 0.04 µg/m$^3$). We will standardize this in a moment
...
of grid search results, consult the documentation for the `grid_search_cv()` function.
...
* Tao et al. (2023) apply PCP to investigate the association between source-specific fine particulate matter
  and myocardial infarction hospitalizations in NYC.
* Wu et al. (2024) employ PCP for exposome profiling of environmental pollutants in seminal plasma, uncovering
  novel associations with semen parameters.
* Benavides et al. (2024) use PCP to develop a Community Severity Index in NYC, measuring the barrier effect of
  road infrastructure and traffic in cities.
