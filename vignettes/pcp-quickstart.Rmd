...
Let's also write a quick helper function that will enable us to visualize the matrices we will be working with throughout this vignette:
...
quick experimentation. Let's use it to simulate a noisy environmental mixture $D = L_0 + S_0 + Z_0$
...
the values as missing `NA` with the `sim_na()` function:
...
measuring the performance of 300 different PCP models. We pass our simulated `lod` vector
as a constant to the gridsearch, equipping each `rrmc()` run with the same LOD information.
To keep this vignette small, we will also set `conserve_memory = TRUE` for the search, which
simply discards metadata that is normally tracked throughout the grid search. 
